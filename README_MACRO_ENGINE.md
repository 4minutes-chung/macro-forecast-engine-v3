# Macro Risk V3 (Frozen Baseline)

## 1) What this package is
Macro Risk V3 is a frozen macro-forecast package for PD input workflows.
It is not a PD model. It produces quarterly macro paths and validation evidence.
...
## 4) Repository map (read this first)
Top-level:
...
- `outputs/macro_engine/bvar_oos_backtest_table.csv` (legacy baseline table retained)
...
## 7) GitHub publish guidance
Recommended commit message pattern:
- `freeze(v3): lock v3.freeze.1 baseline with full audit artifacts`

Recommended tag:
- `v3-freeze-baseline`

Publish policy:
- Do not rewrite V3 artifacts after tag.
- Continue experimentation in V4+ and compare against this baseline.
